...
def inference(yolact_model=None, objdet=None, frame=None):
    global f_shape, f_type, my_lane, imgsz
    ...
    # fps = 30.0
    # frame_tmp = cv2.resize(frame, (1080, 720))
    f_shape = frame.shape
    f_type = frame.dtype
    x_coord, y_coord = PInet_test(lane_agent, frame)
    for i in range(len(x_coord)):                                                                                                
        for j in range(len(x_coord[i])):                                                                                         
            x, y = x_coord[i][j], y_coord[i][j]                                                                                  
            cv2.circle(frame,(int(x), int(y)), 2, (255,255,255), 3)                                                              
