from OT_module.yolact_edge_project.yolact_edge.data import COCODetection, YoutubeVIS, get_label_map, MEANS, COLORS
from OT_module.yolact_edge_project.yolact_edge.data import cfg, set_cfg, set_dataset
from OT_module.yolact_edge_project.yolact_edge.yolact import Yolact
from OT_module.yolact_edge_project.yolact_edge.utils.augmentations import BaseTransform, BaseTransformVideo, FastBaseTransform, Resize
from OT_module.yolact_edge_project.yolact_edge.utils.functions import MovingAverage, ProgressBar
from OT_module.yolact_edge_project.yolact_edge.layers.box_utils import jaccard, center_size
from OT_module.yolact_edge_project.yolact_edge.utils import timer
from OT_module.yolact_edge_project.yolact_edge.utils.functions import SavePath
from OT_module.yolact_edge_project.yolact_edge.layers.output_utils import postprocess, undo_image_transformation
from OT_module.yolact_edge_project.yolact_edge.utils.tensorrt import convert_to_tensorrt
...
def parse_args(argv=None):
    ...
    parser.add_argument('--trained_model',
                        default='OT_module/yolact_edge_project/weights/yolact_edge_resnet50_1467_800000.pth', type=str,
...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45):
    ...
    # Beware: very fast but possibly unintelligiblez mask-drawing code ahead
    # I wish I had access to OpenGL or Vulkan but alas, I guess Pytorch tensor operations will have to suffice
    if args.display_masks and cfg.eval_mask_branch:
        # After this, mask is of size [num_dets, h, w, 1]
        masks = masks[:num_dets_to_consider, :, :, None][0]

        # (contour, cnt) = cv2.findContours(masks.byte().cpu().numpy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    ...
    return (masks_color_summand * 255).byte().cpu().numpy()
...
def evalvideo(net:Yolact, path:str):
    ...
    def get_next_frame(vid):
        ...
        try:
            return [cv2.resize(vid.read()[1],(1080,720)) for _ in range(args.video_multiframe)]
        except cv2.error:
            print("all frame is be read.")
    ...
    def play_video():
        ...
        while running:
            ...
            if cv2.waitKey(20) == 27: # Press Escape to close
                running = False
    ...
    n_threads = 2 # setting thread number
...
def savevideo(net:Yolact, in_path:str, out_path:str):
    ...
    # frame_width  = round(vid.get(cv2.CAP_PROP_FRAME_WIDTH)*0.5)
    # frame_height = round(vid.get(cv2.CAP_PROP_FRAME_HEIGHT)*0.5)
    frame_width = 1080
    frame_height = 720
...
def load_yolact_edge():
    ...
    from OT_module.yolact_edge_project.yolact_edge.utils.logging_helper import setup_logger
