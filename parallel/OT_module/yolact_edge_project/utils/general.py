...
from  yolact_edge_project.utils.torch_utils import init_seeds, is_parallel
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            try:
                ...
                for idx in i:
                    # print(x[idx])
                    tmp_box.append(box[idx].cpu().detach().numpy())
                tmp_box = torch.tensor(tmp_box)
                x = torch.cat((tmp_box, x[i, j + 5, None], j[:, None].float()), 1)
        ...
        # print('11111111111111111111', i, boxes, scores, iou_thres)
        # print('11111111111111111111', i.shape)
        if i.shape[0] > max_det:  # limit detections
            #p#rint('11111111111111111111', i.shape)
            i = i[:max_det]
        ...
        for idx in i:
            # print(x[idx])
            tmp.append(x[idx].cpu().detach().numpy())
