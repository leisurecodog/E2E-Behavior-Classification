...
class Critic(nn.Module):
    def __init__(self, nb_states, nb_actions, hidden1=64, hidden2=32, init_w=3e-3):
...
class Actor_LSTM(nn.Module): # arch  from paper 2014 seq2seq Encoder
    ...
    def __init__(self,
        ...
        self.encode_rnn = nn.LSTM(embedding_size, hidden_size, n_layers,
                           dropout = dropout)
        self.dropout = nn.Dropout(dropout)
        
        self.Flatten = nn.Flatten()
        fn_size = hidden_size * n_layers
        self.linear2 = nn.Linear(fn_size, 2)
    ...
    def forward(self, x):
        ...
        # x: [batch size, input size * len traj]
        ...
        output, (hidden, cell) = self.encode_rnn(embedded)
