...
if __name__ == '__main__':
    ...
    for _ in range(limit):
        data += 1
    print("for without threading:", time.time()-t1)

    import threading
    
    th1 = threading.Thread(target=forer, args=(range(limit//2),))
    th2 = threading.Thread(target=forer, args=(range(limit//2, limit),))
    t1 = time.time()
    th1.start()
    th2.start()
    th1.join()
    th2.join()
    print("for with thread: ", time.time()-t1)